...
from ..strength import symmetric_strength_of_connection, evolution_strength_of_connection, classical_strength_of_connection
from ..relaxation.relaxation import gauss_seidel, gauss_seidel_nr, gauss_seidel_ne, \
                             gauss_seidel_indexed, jacobi, polynomial
from ..relaxation.smoothing import change_smoothers, rho_block_D_inv_A, rho_D_inv_A
from ..krylov import gmres
from ..util.utils import scale_rows, scale_columns, get_Cpt_params, scale_T, amalgamate, \
...
from . import rootnode_solver
from .aggregation.aggregate import standard_aggregation, lloyd_aggregation
from .aggregation.smooth import jacobi_prolongation_smoother, \
                energy_prolongation_smoother, richardson_prolongation_smoother
from .aggregation.tentative import fit_candidates
...
def operator_inner_product(x, y, A):
    ...
    x = numpy.ravel(x)
    y = numpy.ravel(y)
    return numpy.dot( (A*x).conj(), y)

def scale_candidates(B,Bc,A):
    """ Scale B_i and Bc_i by the energy norm of column B_i """

    for j in range(B.shape[1]):
        alpha = operator_inner_product(B[:,j], B[:,j], A) / numpy.dot(B[:,j].conj(), B[:,j])
        B[:,j] /= alpha
        Bc[:,j] /= alpha
    ##
    return B,Bc
...
def cost(smoother):
    """ helper function that determines the cost multipler for the relaxation
    method in prepostsmoother.  For example, the symmetric option for
    gauss_seidel results in a multiplier that is a factor of 2, and number
    of iterations is also a multiplier """
...
def eigen_relaxation_as_linear_operator(A, M, smoother, candidate_iters):
    """
    Return LinearOperator object that applies relaxation based on
    the matrix (A - <Ax, x>/<Mx, x> M) for a homogeneous right-hand-side.
    ...
    if fn == 'gauss_seidel':
        def matvec(x):
            ...
            for j in range(x.shape[0]):
                for k in range(candidate_iters):
                    alpha = 0.0
...
def relaxation_as_linear_operator(A, smoother, candidate_iters):
    """
    Return LinearOperator object that applies relaxation based on
    the matrix A for a homogeneous right-hand-side.
...
def bootamg_rootnode(A,
        ...
        aggregate='standard',
        reaggregate=False,
        smooth=('energy', {'krylov':'cg', 'maxiter':4, 'degree':2, 'weighting':'local'}),
        coarse_solver='pinv2',
        prepostsmoother=('gauss_seidel', {'sweep':'symmetric'}), **kwargs):
    """
    Create a multilevel solver using a bootstrap AMG process and a rootnode-style
    smoothed aggregation framework.
    ...
    if initial_candidates is None:
        U = scipy.zeros((A.shape[0],0), dtype=A.dtype)
    else:
    ...
    if num_new_cands > 0:
        U = numpy.hstack( (U, scipy.rand(U.shape[0], num_new_cands)) )
        if A.dtype == complex:
            U[:,-num_new_cands:] += 1.0j*scipy.rand(A.shape[0], num_new_cands)

    ##
    # Compute indices of candidates to "freeze" and not adaptively updated
    if cands_to_freeze is None:
        not_frozen = numpy.arange(U.shape[1])
    else:
        frozen = numpy.array(cands_to_freeze, dtype=int)
        not_frozen = numpy.setdiff1d(numpy.arange(U.shape[1]), frozen)
    ...
    levels[-1].A = A
    levels[-1].V = numpy.zeros( (A.shape[0],0), dtype=A.dtype)
    levels[-1].U = U
...
def bootamg_setup(levels, l, aggregate, max_coarse, max_levels, strength,
    ...
    """
    Compute additional candidates, and levels, recursively using the bootstrap
    process
    ...
    if len(levels) == max_levels or A.shape[0] <= max_coarse:
        ...
        for i in range( len(levels) -l -1):
            levels.pop()

        ##
        # Do eigensolve and return smallest eigenmodes in V
        [E, V] = scipy.linalg.eig(A.todense(), M.todense() )
        indices = numpy.argsort(numpy.abs(E))
        if symmetry == 'hermitian':
            lvl.V = numpy.real( V[:,indices[:num_eigvects] ] )
        else:
            lvl.V = V[:,indices[:num_eigvects] ]

        print("\nFinished coarsest level %d"%(l) )
        ...
        if len(not_frozen) > 0:
            ...
            for j in not_frozen:
                ...
                if alpha != 0.0: lvl.U[:,j] /= norm(lvl.U[:,j], 'inf')
        ##
        ...
        for mu_i in range(mu):
            ...
            if (not hasattr(lvl, 'T')) or (recoarsen and (mu_i == 0)):
                compute_aggregation = True
            else:
                compute_aggregation  = False

            ##
            # Strength
            if compute_aggregation:
                if(type(strength) == list):  fn, kwargs = unpack_arg(strength[len(levels)-1])
                else: fn, kwargs = unpack_arg(strength)
                if fn == 'symmetric':
                    C = symmetric_strength_of_connection(A, **kwargs)
                    C = C + eye(C.shape[0], C.shape[1], format='csr') # nonzero diagonal
                elif (fn == 'ode') or (fn == 'evolution'):
                    B  = numpy.hstack( (lvl.U,lvl.V) )
                    if 'B' in kwargs:
                    ...
                    raise ValueError('unrecognized strength of connection: %s' % str(fn))
            ...
            # Aggregate
            if compute_aggregation:
                if(type(aggregate) == list):  fn, kwargs = unpack_arg(aggregate[len(levels)-1])
                else: fn, kwargs = unpack_arg(aggregate)
                if fn == 'standard':
            ...
            if compute_aggregation:
                T,dummy_variable = fit_candidates(AggOp,
                                    numpy.ones((A.shape[0],1),dtype=A.dtype) )
            ...
            if fn == 'energy':
                ...
                if False:#hasattr(lvl, 'P') and (not compute_aggregation):
                    ...
                    try:    work[0] += A.nnz*(P.nnz/float(P.shape[0]))*kwargs['maxiter']
                    except: work[0] += A.nnz*(P.nnz/float(P.shape[0]))*4   # 4 is default
            else:
                raise ValueError('unrecognized prolongation smoother: %s' % str(fn))
            ...
            # NOTE: this did not appear in James' code
            if len(not_frozen) > 0:
                work[0] += A.nnz*candidate_iters*len(not_frozen)*cost(prepostsmoother)
            ...
            print("\nFinishing level %d, Grid size (%d,%d)"%(l, A.shape[0], A.shape[1]) )
            if lvl.V.shape[1] > 0:
                init_eigs = numpy.zeros( (lvl.V.shape[1],), dtype=lvl.V.dtype )
                for j in range(lvl.V.shape[1]):
                    init_eigs[j] = operator_inner_product(lvl.V[:,j], lvl.V[:,j], A) / \
                                   operator_inner_product(lvl.V[:,j], lvl.V[:,j], M)
                ...
                for j in range(lvl.V.shape[1]):
                    ...
                    if alpha != 0.0: lvl.V[:,j] /= alpha
                ##
                final_eigs = numpy.zeros( (lvl.V.shape[1],), dtype=lvl.V.dtype )
                for j in range(lvl.V.shape[1]):
                    final_eigs[j] = operator_inner_product(lvl.V[:,j], lvl.V[:,j], A) / \
                                    operator_inner_product(lvl.V[:,j], lvl.V[:,j], M)

                ##
                # Check for eigenvalue convergence
                conv_test = numpy.abs( final_eigs - init_eigs) / numpy.abs(final_eigs)
                print("Relative change in eig estimate:  "+ str(["%1.1e"%ee for ee in conv_test]) )
                print("Eigenvalue estimate:              "+ str(["%1.1e + %1.1e j"%(ee.real,ee.imag) for ee in final_eigs]) )

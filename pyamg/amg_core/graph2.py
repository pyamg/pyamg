...
Iindptr0 = np.array([ 0,  6, 12], dtype=np.int32)
Iindices0 = np.array([ 0,  1,  2,  3,  6,  7,  4,  5,  8,  9, 10, 11], dtype=np.int32)
L0 = np.array([0, 1, 2, 3, 0, 1, 4, 5, 2, 3, 4, 5], dtype=np.int32)
...
np.testing.assert_array_equal(Iindptr, Iindptr0)
np.testing.assert_array_equal(Iindices, Iindices0)
...
c = cluster_center(0,
                   ...
                   cm,
                   I.indptr, I.indices, L)
np.testing.assert_equal(c, 0)
c = cluster_center(1,
                   num_nodes, num_clusters,
                   G.indptr, G.indices, G.data,
                   cm,

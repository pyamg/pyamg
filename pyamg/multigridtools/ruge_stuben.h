...
//    
//    template<class I, class T>
//    void cljp_splitting(const I n_nodes,
//                          const I Sp[],
//                          const I Sj[], 
//                                I splitting[])
//    {
//      std::fill(splitting, splitting + n_nodes, U_NODE);
//    
//      std::vector<T> weight(n_nodes);
//      std::vector<I> Ulist(n_nodes);  // unassigned idx list to nodes [0,Ulistcount]
//      std::vector<I> Ulist_ptr(n_nodes);  // ptr back to index list [0,n_nodes]
//      I Ulistcount = n_nodes;
//      I NewUlistcount;
//      std::vector<I> D(n_nodes);      // marked nodes  in the ind set
//    
//      // INITIALIZE WEIGHTS
//      //std::generate(weight.begin(), weight.end(), drand48());
//      for(I i=0; i < n_nodes; i++){
//        weight[i] = drand48();
//      }
//      for(I i=0; i < n_nodes; i++){
...
//        for(I ii=0; ii<Ulistcount; ii++){
//          I i = Ulist[ii];
//          assert(splitting[i] == U_NODE);
//          if(D[i] > -pass){ // if not already marked in this pass, the initialize
//            D[i] = 1;
//          }
//          for(I jj = Sp[i]; jj < Sp[i+1]; jj++){
//              I j = Sj[jj];
//              if(weight[i]>weight[j]){
//                D[j]=-pass; // mark as not in the ind set in this pass
//              }
//              else if(weight[j]>weight[i]){
//                D[j]=-pass; // mark as not in the ind set in this pass
//              }
//          }
//        }
//    
//        // loop over points in D (and unmarked)
//        NewUlistcount = Ulistcount;
//        for(I ii=0; ii<Ulistcount; ii++){
//          I i = Ulist[ii];
//          // set as C pt and remove from Ulist
//          splitting[i] = C_NODE;
//          Ulist[ii] = Ulist[NewUlistcount-1];
//          Ulist_ptr[Ulist[ii]] = ii;
//          Ulist_ptr[i] = NewUlistcount;
//          NewUlistcount--;
//    
//          // UPDATE WEIGHTS
//          for(I jj = Sp[i]; jj < Sp[i+1]; jj++){
...
//              splitting[j] = F_NODE;
//              I newii = Ulist_ptr[j];
//              Ulist[newii] = Ulist[NewUlistcount-1];
//              Ulist_ptr[Ulist[newii]] = newii;
//              Ulist_ptr[j] = NewUlistcount;
//              NewUlistcount--;
//          }
//        }
//        Ulistcount = NewUlistcount;
//      }
//    }
//    
//    template<class I, class T>
...
//                              const I Tp[], const I Tj[], 
//                                    I splitting[])
//    // CLJP naive since it requires the transpose.  Include this in the timing.
//    //
//    // n         = size square strength matrix S
//    // S         = strength matrix
//    // T         = transpose of strength matrix
//    // splitting = marked as 0 (fine) or coarse (1)
//    {
//    
//      // initialize vectors
//      // complexity = O(n)
...
//              for(I kk = Tp[j]; kk < Tp[j+1]; kk++){
//                I k = Tj[kk];
//                if(splitting[k]==U_NODE){             // k <---j
//                  // does c ---> k ?
//                  for(I ccc = Sp[k]; ccc < Sp[k+1]; ccc++){
//                    if(Sj[ccc]==c){
//                      weight[j]--;
//                      if(weight[j]<1){
//                        splitting[j] = F_NODE;

...
def air_schwarz_solver(A,
    ...
    # Construct Schwarz smoother on each level in hierarchy
    postsmoother = []
    while len(levels) < max_levels and levels[-1].A.shape[0] > max_coarse:
        bottom, psm = extend_hierarchy(levels, strength, CF, interpolation, restrict,
                                  filter_operator, keep)
        postsmoother.append(psm)
        if bottom:
            break

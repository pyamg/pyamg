...
from pyamg.relaxation.relaxation import gauss_seidel, jacobi,\
    ...
    gauss_seidel_nr,\
    jacobi_indexed, cf_jacobi, fc_jacobi, cf_block_jacobi,\
    fc_block_jacobi, cf_gauss_seidel, fc_gauss_seidel
...
class TestRelaxation(TestCase):
    ...
    def test_cf_fc_gauss_seidel(self):
        nf = 200
        nc = 300
        nn = nf+nc
        fpts = np.arange(0,nf)
        ...
        # Compute residual, make sure almost zero
        rfc = bb - FC*xfc
        rcf = bb - CF*xcf
        assert_almost_equal(np.max(np.abs(rfc)), 0)
        assert_almost_equal(np.max(np.abs(rcf)), 0)
        ...
        # Make sure manual FC Gauss-Seidel match PyAMG method
        assert_almost_equal(xfc, xxfc)
        assert_almost_equal(xcf, xxcf)
